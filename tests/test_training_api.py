...
def test_rest_requires_an_api_key(client, monkeypatch):
    ...
    try:
        response = client.post(
            ...
            json={"planned_date": "2026-09-25", "available_minutes": 60,
                  "setting": "outdoor", "discipline": "road"},
        )
...
def test_plan_list_get_and_delete_a_session(client, monkeypatch):
    engine, _ = _shared_factory(monkeypatch)
    monkeypatch.setattr(training_service, "make_training_llm", lambda _key: FakeStructuredLLM(_response()))
    try:
        created = client.post(
            "/api/training/sessions",
            json={"planned_date": "2026-09-25", "available_minutes": 60,
                  "setting": "outdoor", "discipline": "road", "route_idea": "easy spin"},
        )
...
def test_rest_reports_budget_exhaustion(client, monkeypatch):
    ...
    with factory() as db:
        db.add(LLMUsageRecord(model=CHAT_MODEL, prompt_tokens=1, cached_tokens=0, completion_tokens=1, cost_usd=1))
        db.commit()
    try:
        response = client.post(
            "/api/training/sessions",
