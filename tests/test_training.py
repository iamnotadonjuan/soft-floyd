...
from soft_floyd_core.activities.service import ActivitySummaryOut, TrainingSummaryOut, WeekSummaryOut
...
def _profile(**overrides) -> ProfileOut:
    defaults = dict(
        weekly_rides=3, weekly_hours=5.0, goal_text="Gran fondo",
        target_event_name=None, target_event_date=None, focus_areas=[],
        years_riding=5.0, longest_recent_ride_km=100.0, followed_plan_before=True,
        self_rated_level="recreational", available_days=["mon", "wed", "sat"],
        weekday_max_minutes=90, weekend_max_minutes=180,
        birth_year=1990, weight_kg=70.0, max_hr=190, health_notes=None,
        has_hr_monitor=True, ftp_watts=None, lthr=None, workout_devices=["garmin"],
        has_power_meter=False, has_cadence_sensor=False, has_speed_sensor=False,
        primary_discipline="road", capability_tier="hr", available_metrics=[],
    )
...
def _bike(**overrides) -> BikeOut:
    defaults = dict(
        id=1, nickname="Road", kind="road", is_primary=True,
        has_power_meter=False, has_cadence_sensor=False, has_speed_sensor=False,
        capability_tier="hr",
    )
    defaults.update(overrides)
...
def _request(**overrides) -> SessionRequest:
    defaults = dict(
        planned_date=TODAY + dt.timedelta(days=1), available_minutes=60,
        setting="outdoor", discipline="road", route_idea="hill repeats at Patios", feel="normal",
    )
    defaults.update(overrides)
...
def _draft_workout(target: DraftTarget, *, kind="interval") -> DraftWorkout:
    return DraftWorkout(
        ...
        steps=[
            DraftStep(
                kind="warmup", name="Warm up", cue="Easy spin",
                end=StepEnd(kind="time", seconds=600), target=DraftTarget(kind="none"),
            ),
            DraftStep(kind=kind, name="Main", cue="Work", end=StepEnd(kind="time", seconds=600), target=target),
            DraftStep(
                kind="cooldown", name="Cool down", cue="Easy spin",
                end=StepEnd(kind="time", seconds=300), target=DraftTarget(kind="none"),
            ),
        ],
    )
...
def test_power_target_dropped_without_power_meter():
    ...
    workout = sanitize_workout(
        draft, profile=_profile(ftp_watts=250), bike=_bike(has_power_meter=False),
        discipline="road", available_minutes=60,
    )
...
def test_power_target_dropped_without_ftp_even_with_power_meter():
    draft = _draft_workout(DraftTarget(kind="power_pct_ftp", low=90, high=100))
    workout = sanitize_workout(
        draft, profile=_profile(ftp_watts=None), bike=_bike(has_power_meter=True),
        discipline="road", available_minutes=60,
    )
    assert workout.steps[1].target is None
...
def test_power_target_resolved_to_watts():
    draft = _draft_workout(DraftTarget(kind="power_pct_ftp", low=90, high=100))
    workout = sanitize_workout(
        draft, profile=_profile(ftp_watts=250), bike=_bike(has_power_meter=True),
        discipline="road", available_minutes=60,
    )
...
def test_hr_zone_requires_lthr_not_just_a_strap():
    ...
    workout = sanitize_workout(
        draft, profile=_profile(has_hr_monitor=True, lthr=None), bike=_bike(),
        discipline="road", available_minutes=60,
    )
    assert workout.steps[1].target is None
...
def test_hr_zone_resolved_from_lthr_table():
    ...
    workout = sanitize_workout(
        draft, profile=_profile(has_hr_monitor=True, lthr=160), bike=_bike(),
        discipline="road", available_minutes=60,
    )
    target = workout.steps[1].target
...
def test_cadence_target_needs_a_cadence_sensor():
    ...
    workout = sanitize_workout(
        draft, profile=_profile(), bike=_bike(has_cadence_sensor=False),
        discipline="road", available_minutes=60,
    )
    assert workout.steps[1].target is None
    workout = sanitize_workout(
        draft, profile=_profile(), bike=_bike(has_cadence_sensor=True),
        discipline="road", available_minutes=60,
    )
...
def test_no_matching_bike_means_no_sensor_targets():
    draft = _draft_workout(DraftTarget(kind="power_pct_ftp", low=90, high=100))
    workout = sanitize_workout(
        draft, profile=_profile(ftp_watts=250), bike=None, discipline="road", available_minutes=60,
    )
    assert workout.steps[1].target is None
...
def test_duration_clamp_shrinks_intervals_not_warmup_or_cooldown():
    draft = DraftWorkout(
        name="Long", est_minutes=90,
        steps=[
            DraftStep(kind="warmup", name="Warm up", cue="Easy", end=StepEnd(kind="time", seconds=600), target=DraftTarget(kind="none")),
            DraftRepeatBlock(count=6, steps=[
                DraftStep(kind="interval", name="Hard", cue="Hard", end=StepEnd(kind="time", seconds=300), target=DraftTarget(kind="none")),
                DraftStep(kind="recovery", name="Easy", cue="Easy", end=StepEnd(kind="time", seconds=180), target=DraftTarget(kind="none")),
            ]),
            DraftStep(kind="cooldown", name="Cool down", cue="Easy", end=StepEnd(kind="time", seconds=600), target=DraftTarget(kind="none")),
        ],
    )
    workout = sanitize_workout(draft, profile=_profile(), bike=_bike(), discipline="road", available_minutes=30)
    ...
    assert workout.est_minutes < 68  # meaningfully closer to the 30-minute ask than the 68-minute draft


def test_repeat_count_is_clamped():
    draft = DraftWorkout(
        name="Silly", est_minutes=10,
        steps=[DraftRepeatBlock(count=999, steps=[
            DraftStep(kind="interval", name="X", cue="X", end=StepEnd(kind="time", seconds=10), target=DraftTarget(kind="none")),
        ])],
    )
    workout = sanitize_workout(draft, profile=_profile(), bike=_bike(), discipline="road", available_minutes=60)
...
def _summary(hours_this_week: float = 0.0) -> TrainingSummaryOut:
    week = WeekSummaryOut(
        week_start=TODAY - dt.timedelta(days=TODAY.weekday()), rides=1,
        distance_km=30.0, duration_h=hours_this_week, elev_gain_m=200.0,
        avg_hr=None, hr_rides=0, avg_power_w=None, power_rides=0,
    )
    return TrainingSummaryOut(weeks=[week], total_rides=1, total_distance_km=30.0, total_duration_h=hours_this_week, data_note=None)


def _ride(days_ago: int, duration_s: float) -> ActivitySummaryOut:
    return ActivitySummaryOut(
        id=1, start_time=NOW - dt.timedelta(days=days_ago), sport="cycling", sub_sport="",
        bike_type="road", is_indoor=False, distance_m=30000.0, duration_s=duration_s,
        elev_gain_m=100.0, avg_hr=None, max_hr=None, avg_power_w=None, avg_cadence=None,
        sensors_present=[], fit_status="ok",
    )
...
def test_off_schedule_day_is_flagged():
    ...
    intent = recommend_intent(_request(planned_date=TODAY + dt.timedelta(days=1)), profile, _summary(), [], today=TODAY)
...
def test_fit_export_encodes_a_repeat_block_correctly(tmp_path):
    draft = DraftWorkout(
        name="Repeats", est_minutes=30,
        steps=[
            DraftStep(kind="warmup", name="Warm up", cue="Easy", end=StepEnd(kind="time", seconds=300), target=DraftTarget(kind="none")),
            DraftRepeatBlock(count=4, steps=[
                DraftStep(kind="interval", name="Hard", cue="Hard", end=StepEnd(kind="time", seconds=60), target=DraftTarget(kind="power_pct_ftp", low=100, high=110)),
                DraftStep(kind="recovery", name="Easy", cue="Easy", end=StepEnd(kind="time", seconds=60), target=DraftTarget(kind="none")),
            ]),
        ],
    )
    workout = sanitize_workout(draft, profile=_profile(ftp_watts=200), bike=_bike(has_power_meter=True), discipline="road", available_minutes=30)
...
class FakeStructuredLLM:
    ...
    async def chat_structured(self, messages, schema_name, schema, *, max_completion_tokens, temperature=0):
    ...
    return {
        "workout": {
            ...
            "steps": [
                ...
                {"kind": "interval", "name": "Climb", "cue": "Hard, seated",
...
def _seed(db_session, *, ftp_watts=None, power_bike=False):
    db_session.add(RiderProfile(
        id=1, goal_text="Gran fondo", has_hr_monitor=True, lthr=160,
        ftp_watts=ftp_watts, available_days=["mon", "wed", "sat"],
        weekday_max_minutes=90, weekend_max_minutes=180, focus_areas=["climbing"],
    ))
...
async def test_plan_session_drops_power_target_for_a_no_power_rider(db_session):
    ...
    llm = FakeStructuredLLM(_generated_response({"kind": "power_pct_ftp", "low": 95, "high": 105, "hr_zone": None}))
    request = SessionRequest(planned_date=TODAY + dt.timedelta(days=1), available_minutes=60,
                              setting="outdoor", discipline="road", route_idea="hill repeats", feel="normal")

    out = await training_service.plan_session(db_session, llm, None, request, budget_usd=10.0, now=NOW)
...
async def test_plan_session_keeps_power_target_for_a_power_rider(db_session):
    ...
    llm = FakeStructuredLLM(_generated_response({"kind": "power_pct_ftp", "low": 90, "high": 100, "hr_zone": None}))
    request = SessionRequest(planned_date=TODAY + dt.timedelta(days=1), available_minutes=60,
                              setting="outdoor", discipline="road", route_idea="hill repeats", feel="normal")
...
async def test_plan_session_over_budget_raises(db_session):
    ...
    _seed(db_session)
    llm = FakeStructuredLLM(_generated_response({"kind": "none", "low": None, "high": None, "hr_zone": None}))
    request = SessionRequest(planned_date=TODAY, available_minutes=60, setting="outdoor", discipline="road")
...
async def test_list_status_and_delete(db_session):
    ...
    llm = FakeStructuredLLM(_generated_response({"kind": "none", "low": None, "high": None, "hr_zone": None}))
    request = SessionRequest(planned_date=TODAY, available_minutes=60, setting="outdoor", discipline="road")
    created = await training_service.plan_session(db_session, llm, None, request, budget_usd=10.0, now=NOW)
...
async def test_send_to_garmin_stores_the_returned_workout_id(db_session):
    ...
    out2 = await training_service.send_to_garmin(db_session, created.id, runner)
