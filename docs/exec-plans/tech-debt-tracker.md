...
| Garmin tokens are plaintext per-account files | 0009 keys `garmin_tokens.json` by account directory (`garmin_token_dir/<id>/`) but still stores it unencrypted, fine on loopback only | Before any remote deployment (same trigger as the row above): move tokens into the DB encrypted, e.g. Fernet with a key from env |
| Garmin poller starts a task for every account | `lifespan.py`'s supervisor polls `Account` and starts one `run_forever` task per row, including accounts that never connected Garmin | If idle polling gets costly: only start a task once that account's token file exists |
